  ...
  it("Should send multiple B2C payments in parallel and receive results or timeout callback", function (done) {
    ...
    Promise.all([
      ...
      .then(([responseBody1, responseBody2, responseBody3]) => {
        [responseBody1, responseBody2, responseBody3].forEach((responseBody, index) => {
          expect(responseBody).to.be.an("object");
          expect(responseBody?.b2cRequestResponse.ResponseCode).to.be.equal("0");
          console.log("\x1b[35m\x1b[4m\x1b[1m%s\x1b[0m", `Test ${index + 1} passed successfully`);
